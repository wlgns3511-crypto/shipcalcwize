  ...
  return (
    <article>
      ...
      <nav className="text-xs text-slate-500 mb-6">
        ...
        <Link href="/insights/" className="hover:text-amber-600">Insights</Link>
        <span className="mx-1">&rsaquo;</span>
      ...
      <header className="mb-8">
        <div className="flex items-center gap-2 mb-3">
          <span className="text-xs px-2 py-0.5 bg-amber-50 text-amber-600 border border-amber-200 rounded-full">Insight</span>
      ...
      <div className="prose prose-slate max-w-none mb-8 prose-p:leading-relaxed prose-p:text-slate-700 prose-a:text-amber-600 prose-a:no-underline hover:prose-a:underline prose-strong:text-slate-900" dangerouslySetInnerHTML={{ __html: article.content }} />

      <div className="my-8 p-5 rounded-xl bg-amber-50 border border-amber-200">
        <p className="text-xs font-bold uppercase tracking-wider text-amber-600 mb-2">Key Takeaway</p>
      ...
      {others.length > 0 && (
        <section className="mt-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {others.map((a) => (
              <Link key={a.slug} href={`/insights/${a.slug}/`} className="block border border-slate-200 rounded-xl p-4 hover:border-amber-300 hover:shadow-sm transition-all group">
                <span className="text-xs text-slate-400">{a.date}</span>
                <h3 className="text-sm font-semibold text-slate-900 group-hover:text-amber-700 mt-1 leading-snug">{a.title}</h3>
              </Link>
            ))}
          </div>
        </section>
